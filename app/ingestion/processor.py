    ...
    with logfire.span("Processing file", file=filename, source=source_type):
        try:
            ...
            if ext == "pdf":
                ...
                logfire.warning("Unsupported file type: {filename}")
                return

            if not full_text or not full_text.strip():
                logfire.warning("No text extracted from file: {filename}")
                return

            # Chunk the text
            chunks = chunk_text(full_text)
            if not chunks:
                logfire.warning("No chunks created for file: {filename}")
                return
            ...
            logfire.info("Processed data saved locally at {local_path}")

            with logfire.span(
                "Vectorizing and indexing chunks", file=filename, source=source_type
            ):
                ...
                logfire.info(
                    f"Indexed {len(points)} chunks in Qdrant for file: {filename}"
                )
...
def process_directory(dir_path: str, source_type: str):
    ...
    with logfire.span("Scanning directory", path=dir_path, source=source_type):
        ...
        logfire.info(f"Found {len(files)} in {dir_path}")
...
def run_universal_ingestion(
    ...
    with logfire.span("Universal Ingestion started", base_directory=base_dir):
        if not qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
            ...
            logfire.info(
                f"Created collection'{settings.QDRANT_COLLECTION}'({dim}-dim,Cosine)."
            )
        ...
        if not subdirs:
            ...
            logfire.info(
                f"No sub-folders fuond for processing '{base_dir} as '{source_type}'."
            )
